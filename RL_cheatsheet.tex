...
The parameterization can be done by any kind of estimator but we typically use a neural network.\\
...
\begin{algorithm}[H]
 ...
 \ForEach{episode $1\leqslant k \leqslant n$}{
    ...
    \For{$t = T_k, T_{k-1}, ..., 1$}{
        ...
        $\theta_{k+1} = \theta_k + \alpha \sum_{t=1}^{T} \nabla_{\theta_k} \log \pi_{\theta_k} ( s_{t,k}, a_{t,k})  A^{\theta_k}_{\Phi_k}(s_{t,k}, a_{t,k})$\\
        Update the critic network (typically with MSE):
        $\Phi_{k+1} = \Phi_k + \left ( R_t(\tau_k) - V_{\Phi_k}(s_{t,k}) \right ) \nabla_{\Phi_k} V_{\Phi_k}(s_{t,k})$
    }
 }    
\caption{A2C}
\end{algorithm}
